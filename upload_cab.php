...
if(isset($_FILES['file']) && $_FILES['file']['error'] == 0){
	...
	$id 	= $codigo;
    $assoc 	= $cabeca->getID($id);
	unlink("img/miniaturas/$assoc->thumb");
	unlink("img/$assoc->foto");
	...
	if(!in_array(strtolower($extension), $allowed)){
		...
		$upload 	= move_uploaded_file($arqTemp, $targetFile);
      	...
      	$imgsize 	= getimagesize($targetFile);
			...
			$g_width 	= 800; //Nova largura da imagem
			$g_height 	= $imgsize[1]/$imgsize[0]*$g_width; //para manter a proporção

			$width 		= 150; //Largura da miniatura    
